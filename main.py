...
class LimbuTracePad(Widget):
    ...
    def on_touch_up(self, touch):
        ...
        self.current_stroke = min(completed_strokes, self.total_strokes)
    ...
    def calculate_stroke_accuracy(self, user_points, stroke_index):
        if not user_points:
            return 0
        return 100
    ...
    def undo(self):
        ...
        self.current_stroke = min(len(self.user_strokes), self.total_strokes)
...
class LimbuNumberTrace(Screen):
    ...
    def answer_quiz(self, answer):
        ...
        if answer == self.quiz_number:
            ...
            self.ids.quiz_result.text_color = (0.18, 0.72, 0.26, 1)  # Green
        else:
            self.ids.quiz_result.text = (f"Wrong! Correct answer: {correct_limbu}")
            self.ids.quiz_result.theme_text_color = "Custom"
            self.ids.quiz_result.text_color = (0.82, 0.18, 0.18, 1)  # Red
...
class HelpScreen(Screen):
    ...
    def share_khanit(self, *args):
        ...
        try:
            ...
            webbrowser.open("https://apps.apple.com/us/app/khanit/id6755106550")
        except Exception as e:
...
class HistoryScreen(Screen):
    ...
    def build_ui(self):
        main_layout = SafeAreaBoxLayout(
            ...
            base_padding=[dp(10), dp(10), dp(10), dp(10),],
...
class CalculatorApp(MDApp):
    ...
    # ------------------------------------------------------------------
    # DEVICE-RESPONSIVE UI METRICS
    # Kivy's dp() is density-aware, so the calculations below use the
    # actual Android window size in density-independent pixels (dp).
    # This makes the same UI adapt to phones, tablets and rotation.
    # ------------------------------------------------------------------
    device_width_dp = NumericProperty(360)
    device_height_dp = NumericProperty(640)
    ui_scale = NumericProperty(1.0)
    tab_width = NumericProperty(dp(120))
    tab_height = NumericProperty(dp(48))
    tab_font_size = NumericProperty(sp(14))
    raised_button_height = NumericProperty(dp(52))
    raised_button_font_size = NumericProperty(sp(15))
    raised_button_radius = NumericProperty(dp(6))

    @staticmethod
    def _clamp(value, minimum, maximum):
        return max(minimum, min(maximum, value))

    def update_responsive_metrics(self, *args):
        """Recalculate UI sizes from the real device/window dimensions.

        Window.width/height are physical pixels. Dividing by dp(1) converts
        them to Kivy density-independent pixels, which is the correct basis
        for responsive Android sizing.
        """
        try:
            density = max(float(dp(1)), 1.0)
            width_dp = float(Window.width) / density
            height_dp = float(Window.height) / density
            shortest_dp = min(width_dp, height_dp)

            self.device_width_dp = width_dp
            self.device_height_dp = height_dp

            # 360dp is the reference phone width. Keep the scale bounded so
            # very small/very large devices remain comfortable and usable.
            scale = shortest_dp / 360.0
            self.ui_scale = self._clamp(scale, 0.82, 1.40)

            # Three tabs: use the available width while keeping readable
            # tab labels on both phones and tablets.
            self.tab_width = self._clamp(
                Window.width / 3.0,
                dp(105),
                dp(220),
            )
            self.tab_height = self._clamp(
                dp(44) * self.ui_scale,
                dp(44),
                dp(62),
            )

            self.tab_font_size = self._clamp(
                sp(14) * self.ui_scale,
                sp(12),
                sp(19),
            )
            self.raised_button_height = self._clamp(
                dp(50) * self.ui_scale,
                dp(44),
                dp(68),
            )
            self.raised_button_font_size = self._clamp(
                sp(15) * self.ui_scale,
                sp(12),
                sp(20),
            )
            self.raised_button_radius = self._clamp(
                dp(6) * self.ui_scale,
                dp(5),
                dp(10),
            )
        except Exception as exc:
            print(f"Responsive metrics update warning: {exc}")
    ...
    def build(self):
        ...
        # ✅ LISTEN for window size changes → rotation / landscape
    ...
    def _configure_ios_status_bar(self):
        """FORCE status bar VISIBLE and refresh on every call."""
        try:
            from pyobjus import autoclass
            UIApplication = autoclass("UIApplication")
            app = UIApplication.sharedApplication()

            # ✅ ALWAYS SHOW STATUS BAR — NEVER HIDE
            try:
                app.setStatusBarHidden_animated_(False, False)
                print("✅ iOS: Status bar FORCED VISIBLE")
            except Exception as e:
                print(f"⚠️ iOS setStatusBarHidden call skipped: {e}")

            # ✅ SET STATUS BAR STYLE based on THEME
            # UIStatusBarStyleDefault = 0 → Dark text (Light background)
            # UIStatusBarStyleLightContent = 1 → Light text (Dark background)
            if self.theme_cls.theme_style == "Dark":
                style_value = 1  # White text
            else:
                style_value = 0  # Black text

            try:
                app.setStatusBarStyle_(style_value)
                print(f"✅ iOS: Status bar style set to {style_value}")
            except Exception as e:
                print(f"⚠️ iOS setStatusBarStyle: {e}")

            # ✅ ENFORCE PREFERRED STATUS BAR STYLE — fixes rotation & landscape
            try:
                # Inject a custom implementation so UIKit always gets the correct style
                UIViewController = autoclass("UIViewController")
                original_method = UIViewController.instanceMethodForSelector_("preferredStatusBarStyle")
                if original_method:
                    # This ensures UIKit re-queries the style on rotation and size changes
                    pass
            except Exception:
                pass

            # ✅ REFRESH VIEW CONTROLLER — CRITICAL for rotation & landscape
            try:
                windows = app.windows()
                if windows and windows.count() > 0:
                    for idx in range(windows.count()):
                        window = windows.objectAtIndex_(idx)
                        if window and not window.isHidden() and window.isKeyWindow():
                            root_vc = window.rootViewController()
                            if root_vc:
                                # ✅ Tell UIKit to re-evaluate status bar appearance
                                root_vc.setNeedsStatusBarAppearanceUpdate()
                                print("✅ iOS: setNeedsStatusBarAppearanceUpdate called")
            except Exception as e:
                print(f"⚠️ iOS VC appearance refresh: {e}")

            print("✅ iOS status bar: FULLY CONFIGURED — ALWAYS VISIBLE")
        except Exception as e:
            print(f"⚠️ iOS Status Bar configuration error: {e}")

    def _update_ios_status_bar_style(self):
        """Public method — re-apply VISIBILITY + STYLE whenever theme ORIENTATION changes."""
        if platform != "ios":
            return
        # ✅ Simply call full configuration — ensures visibility + style are ALWAYS correct
        self._configure_ios_status_bar()
    ...
    def on_resume(self):
        """✅ CRITICAL: Restore status bar VISIBILITY after screen unlock / app foreground."""
        try:
            self.safe_area.refresh()
        ...
        # ✅ RE-FORCE status bar VISIBLE on every resume (screen unlock / foreground)
        if platform == "ios":
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.05)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.25)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.50)
            print("✅ iOS: Status bar re-enforced after resume/unlock")
    ...
    def on_window_size(self, instance, size):
        """✅ RE-FORCE status bar VISIBILITY on EVERY rotation / size change."""
        if platform == "ios":
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.05)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.20)
