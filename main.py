...
class CalculatorApp(MDApp):
    ...
    def _configure_ios_status_bar(self):
        """Force status bar ALWAYS visible — called on start, resume, AND rotation."""
        if platform != "ios":
            return
        try:
            from pyobjus import autoclass
            ...
            # ✅ NEVER hide status bar — FORCE visible
            app.setStatusBarHidden_animated_(False, False)

            # ✅ Choose text color based on theme
            style_value = 1 if self.theme_cls.theme_style == "Dark" else 0
            ...
            # ✅ Force VC to refresh status bar appearance
            try:
                windows = app.windows()
            ...
            print(f"✅ StatusBar FORCED visible, style={style_value}")
        except Exception as e:
            print(f"⚠️ iOS status bar config error: {e}")

    def _update_ios_status_bar_style(self):
        """Update text color WITHOUT hiding — keep visible always."""
        self._configure_ios_status_bar()  # Reuse full visibility logic
    ...
    def on_window_resize(self, window, width, height):
        """✅ CRITICAL: Re-force status bar visibility AFTER rotation."""
        if platform == "ios":
            # Re-apply visibility MULTIPLE times after rotation
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.05)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.15)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.30)
            Clock.schedule_once(lambda dt: self._configure_ios_status_bar(), 0.50)
        # Also refresh safe-area insets
        Clock.schedule_once(self._refresh_safe_area, 0.05)
        Clock.schedule_once(self._refresh_safe_area, 0.20)
    ...
    def on_resume(self):
        """Re-apply EVERYTHING on resume — including visibility."""
        try:
            self.safe_area.refresh()
    ...
    def _on_theme_style_changed(self, instance, theme_style):
        ...
        Window.clearcolor = self.theme_background
        if platform == "ios":
            self._update_ios_status_bar_style()  # Updates text color but KEEPS bar visible
        try:
